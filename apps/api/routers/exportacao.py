...
def download_pacote_exportacao(
    ...
    if pacote is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Pacote de entrega nao encontrado."
        )

    if pacote.status is not StatusExportacao.CONCLUIDO or not pacote.caminho_arquivo:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Pacote de entrega ainda nao esta concluido.",
        )

    caminho_zip = Path(pacote.caminho_arquivo)
    if not caminho_zip.exists():
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Ficheiro ZIP nao encontrado no disco."
        )
